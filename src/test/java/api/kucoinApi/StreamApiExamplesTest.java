...
//        Assert.assertEquals(top10.get(0).getSymbol(), "");
    }
...
//       Map<String, Float> usd = new HashMap<>();
   ...
   //     getTickers().forEach(x -> usd.put(x.getSymbol(),Float.parseFloat((x.getChangeRate()))));
        List<TicketShort> shortList = new ArrayList<>();
        getTickers().forEach(x -> shortList.add(new TicketShort(x.getSymbol(), Float.parseFloat(x.getChangeRate()))));
    }
}
